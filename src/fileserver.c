	...
	while (EZN_TRUE) {
		...
		if (_kbhit()) {
			...
			if (ch == '\b' || ch == 127) {
				...
				char process_cmd[PACKETSIZE+10];
				...
				if (strcmp(input_buffer, "exit") == 0) {
					...
					process_cmd[0] = 'a';
					process_cmd[1] = 'e';
					if (ezn_send(clientsock, (EZN_BYTE*)(process_cmd), strlen(process_cmd) + 1, &returnlen) == EZN_ERROR) {
						EZN_WARN("send failed");
					} else {
						if (ezn_recieve(clientsock, out_cmd, 3, &returnlen) == EZN_ERROR) {
...
void net_handler(void* params) {
	EZN_SOCKET serversock = ((Handler_args*)params)->client_socket;
...
EZN_STATUS server_behavior(ezn_Server* server, EZN_SOCKET clientsock) {
	...
	netargs.client_socket = clientsock;
